    ...
    public static function allocateMany(array $pallets): bool 
    {
        ...
        foreach($pallets as $pallet) {
            if(!$pallet->isStored() && !$pallet->isSeparated() && !$pallet->isReleased()) {
                ...
                $pallet->street_number = $availablePlaces[$profile][$i[$profile]]['street_number'];
                $pallet->position = $availablePlaces[$profile][$i[$profile]]['position'];
                $pallet->height = $availablePlaces[$profile][$i[$profile]]['height'];
